...
1. `Add translations for configurable text values`_ in the pygeoap configuration;
...
1. Extract from latest code the keys to be translated. These keys are captured in a `.pot` file.  Note that the `.pot` file is not to be stored in version control, but as an intermidiary file used to update `/locale/*/LC_MESSAGES/messages.po` files:
